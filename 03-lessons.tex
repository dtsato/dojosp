...
\item \textbf{``What has hindered learning?''}: The negative aspects
  ...
  eliminated and come up with a series of action items. People are assigned
...
We tried to fight that and it got a bit better with time but it is no
longer a rule. It became more likely a good practice that the moderator should
...
nodes on a graph are the appropriate solution to the problem. At times,
